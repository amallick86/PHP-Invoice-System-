...
<?php
	$q="SELECT users.uid, users.uname, tbl_orderdetails.id, tbl_orderdetails.order_id, tbl_orderdetails.vendor_id,
			tbl_orderdetails.vendorname, tbl_orderdetails.date, tbl_orderdetails.Product_name, tbl_orderdetails.amountdr,
			tbl_orderdetails.amountcr,tbl_orderdetails.submittedby
FROM tbl_orderdetails
INNER JOIN users ON tbl_orderdetails.submittedby=users.uid  ORDER BY id desc";
	if(isset($_POST["submit"]))
	{
			...
			tbl_orderdetails.amountcr,tbl_orderdetails.submittedby
FROM tbl_orderdetails
INNER JOIN users ON tbl_orderdetails.submittedby=users.uid WHERE $searchby ='$search' OR date BETWEEN '$fdate' AND '$tdate' ORDER BY date DESC";
	}
...
<!--tr><td>Sn</td><td>First Name</td><td>Last Name</td><td>gender</td><td>Customer ID</td><td>Email</td><td>Phone</td><td>Mobile</td><td>Company</td><td>Branch</td></tr>-->
	<thead>
		<tr>
			...
			<th><strong>Subbmited by</strong></th>
...
while($row=mysqli_fetch_assoc($result1)) {  ?>
	...
	<tr>
		...
		<td align="center"><a href="editbill.php?id=<?php echo $row["id"]; ?>"><?php echo $row["order_id"]; ?></a></td>
		<td align="center"><a href="editbill.php?id=<?php echo $row["id"]; ?>"><?php echo $row["vendor_id"]; ?></a></td>
		<td align="center"><a href="editbill.php?id=<?php echo $row["id"]; ?>"><?php echo $row["vendorname"]; ?></a></td>
		...
		<!--td align="center"><a href="pdf/pdf.php?id=<!--?php echo $row["id"]; ?>">PDF</a></td-->
	</tr>
<?php $sn++;
}
?>
